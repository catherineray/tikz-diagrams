...
\usepackage{subcaption, listings, courier, fancyhdr}
...
\begin{tikzpicture}
  ...
  \draw[->] (X) to node[above] {$id_A$} (X1);
  \draw[->] (X1) to node[right] {$f$} (Y1);
	\draw [->] (Y) to node[below] {$id_B$} (Y1) ;
\end{tikzpicture}
...
\begin{tikzpicture}
  ...
  \node (Y) [below of=X] {$C$};
  \node (Y1) [right of=Y] {$D$};
  \draw[->] (X) to node[left] {$g \circ f$} (Y);
  \draw[->] (X1) to node[left] {$g$} (Y);
  \draw[->] (X) to node[above] {$f$} (X1);
  \draw[->] (X1) to node[right] {$h \circ g$} (Y1);
	\draw [->] (Y) to node[below] {$h$} (Y1) ;
\end{tikzpicture}
...
\begin{tikzpicture}
  \node (X) {$\mathbb{R}$};
  \node (X1) [right of=X] {$S^1$};
  \node (Y) [below of=X] {$\mathbb{R}/\tau\mathbb{Z}$};
  \draw[->] (X) to node {$e^{i\theta}$} (X1);
  \draw[dashed, <-] (X1) to node {$\simeq$} (Y);
  \draw[->] (X) to (Y);
\end{tikzpicture}
